/**
 * Test if dividing by (N-1)² fixes the scaling issue
 */
...
console.log("\n=== Testing Scaling Fix: Divide by (N-1)² ===\n");
...
function testForN(N: number, useFix: boolean): number {
  const a = -L / 2;
  const b = L / 2;
  ...
  let scaling = Math.pow(2 / (b - a), 2);

  // TEST THE FIX: divide by (N-1)²
  if (useFix) {
    scaling = scaling / Math.pow(N - 1, 2);
  }
