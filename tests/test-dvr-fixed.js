/**
 * Test DVR with CORRECT grid spacing (cell-centered, no boundaries)
 */
...
function testDVR_FIXED() {
  console.log('\n=== DVR with CORRECT Grid (Cell-Centered, No Boundaries) ===\n');
  ...
  // CORRECTED: Grid spacing without -1, cell-centered
  const dx = L / numPoints;

  // Generate cell-centered grid points (NO boundaries)
  const xGrid = [];
  for (let i = 0; i < numPoints; i++) {
    // x_i = xMin + (i + 0.5) * dx
    xGrid.push(xMin + (i + 0.5) * dx);
  }

  console.log(`Grid spacing: dx = ${dx.toExponential(4)} m`);
  console.log(`Number of points: ${numPoints}`);
  console.log(`First grid point: ${xGrid[0].toExponential(4)} m (should be > xMin)`);
  console.log(`Last grid point: ${xGrid[numPoints-1].toExponential(4)} m (should be < xMax)`);
