...
for (let state = 0; state < 3; state++) {
  ...
  console.log(
    `        Parity: ${parity} (expected ${expected}) ${correct ? "✓" : "✗ INCORRECT"}`,
  );
}
...
console.log(
  `  Expected (≈ 2/a₀^(3/2)): ${(2 / Math.pow(A_0, 1.5)).toExponential(4)}`,
);
...
const antisymmetryError =
  Math.abs(leftValSym + rightValSym) /
  Math.max(Math.abs(leftValSym), Math.abs(rightValSym));
