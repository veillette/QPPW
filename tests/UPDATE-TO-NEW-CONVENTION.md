...
// NEW:
assert(groundState.energy > 0 && groundState.energy < 5.0, 'Ground state energy should be bound (0 < E < wellDepth)');
```
...
// NEW:
assert(groundState.energy > 0 && groundState.energy < wellDepth, `Ground state should be bound (0 < E < ${wellDepth} eV)`);
```
...
// NEW:
console.log(`  Ground state energy: ${groundState.energy.toFixed(6)} eV (0 < E < ${5.0} eV)`);
```
