...
globalThis.phet.chipper = globalThis.phet.chipper || {
  packageObject: { name: "scenerystack" },
  queryParameters: {},
};
...
async function debug() {
  ...
  console.log(
    `Expected E_0 = ${((HBAR * omega * 0.5) / EV_TO_JOULES).toFixed(6)} eV`,
  );
  console.log("");
  ...
  console.log(
    `  H[N/2,N/2] = ${H.get(Math.floor(N / 2), Math.floor(N / 2)).toExponential(3)}`,
  );
  ...
  try {
    ...
    console.log(
      `  (S^-1*H)[N/2,N/2] = ${SinvH.get(Math.floor(N / 2), Math.floor(N / 2)).toExponential(3)}`,
    );
    console.log("");
    ...
    for (let i = 0; i < Math.min(10, sortedEigs.length); i++) {
      ...
      console.log(
        `  λ_${i} = ${sortedEigs[i].toExponential(4)} J = ${eEV.toFixed(6)} eV`,
      );
    }
    console.log("");
    ...
    console.log(
      `Bound states (E < ${(V_boundary / EV_TO_JOULES).toFixed(2)} eV): ${boundStates.length}`,
    );
  } catch (err) {
